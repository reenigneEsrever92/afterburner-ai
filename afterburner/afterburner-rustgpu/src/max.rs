...
use afterburner_rustgpu_shared::{max::RustGpuMaxParams, RustGpuShape};

use crate::{run_with_backend, RustGpu};

impl<const D: usize> MaxImpl<RustGpu, f32, D> for RustGpu {
    fn max(input: &Tensor<RustGpu, D, f32>, params: MaxParams) -> Tensor<RustGpu, D, f32> {
...
mod test {
    ...
    fn test_max_global_1d() {
        ...
        let result = input.max_global().unwrap();
    ...
    #[test]
    fn test_max_along_dimension_2d() {
        init();
        ...
        let result = input.max(0i32).unwrap();
        let output = result.to_vec();

        assert_eq!(output.len(), 3);
        assert_eq!(output[0], 4.0); // max of [1, 4]
        assert_eq!(output[1], 2.0); // max of [2, 0]
        assert_eq!(output[2], 6.0); // max of [3, 6]
    }

    #[test]
    fn test_max_along_dimension_with_keep_dims() {
        init();

        let input: Tensor<RustGpu, 2, f32> =
            RustGpu::new_tensor(Shape([2, 3]), vec![1.0, 2.0, 3.0, 4.0, 0.0, 6.0]);

        // Max along dimension 1 (columns) with keep_dims=true
        let result = input.max((1i32, true)).unwrap();
        let shape = result.shape().to_owned();
        let output = result.to_vec();

        assert_eq!(shape, Shape([2, 1])); // Shape preserved with dim=1
        assert_eq!(output.len(), 2);
        assert_eq!(output[0], 3.0); // max of [1, 2, 3]
        assert_eq!(output[1], 6.0); // max of [4, 0, 6]
    }
    ...
    #[test]
    fn test_max_convenience_methods() {
        init();
        ...
        let result1 = input.max(0i32).unwrap(); // From i32
        let result2 = input.max((0i32, false)).unwrap(); // From (i32, bool)
        let result3 = input.max(Some(0i32)).unwrap(); // From Option<i32>
        ...
        assert_eq!(out1, out2);
        assert_eq!(out1, out3);
        assert_eq!(out1[0], 3.0);
        assert_eq!(out1[1], 4.0);
    }
}
