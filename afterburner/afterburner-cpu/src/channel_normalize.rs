...
impl ChannelNormalizeImpl<Cpu, f32> for Cpu {
    fn channel_normalize(
        input: &Tensor<Cpu, 4, f32>,
        params: ChannelNormalizeParams,
    ) -> Tensor<Cpu, 4, f32> {
        let input_shape = input.shape();
        let [batch_size, channels, height, width] = input_shape.0;
        ...
        let batch_stride = channels * channel_stride;

        // Apply channel normalization: (input - mean) / std for each channel
        for batch in 0..batch_size {
            for channel in 0..channels {
                ...
                for spatial_idx in 0..spatial_size {
                    let idx = batch * batch_stride + channel * channel_stride + spatial_idx;
        ...
        Self::new_tensor(*input_shape, output_data)
    }
}

#[cfg(test)]
mod test {
    use afterburner_core::{prelude::Shape, tensor::Tensor};
    ...
    fn test_channel_normalize_rgb() {
        ...
        // Use typical normalization values (scaled for easier testing)
    ...
    fn test_channel_normalize_convenience() {
        let input: Tensor<Cpu, 4, f32> = Cpu::new_tensor(Shape([1, 2, 1, 1]), vec![10.0, 20.0]);
